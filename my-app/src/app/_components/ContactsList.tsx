  ...
  return (
    <div className=" max-w-md ">
        {contacts.map(contact => (
            <div key={contact._id} className="px-4 py-2 rounded-md mb-4
             flex items-start justify-between gap-4 border border-gray-200 shadow-sm">
                <div>
                    <h3><b>Name:</b> {contact.name}</h3>
                    <p><b>Email:</b> {contact.email}</p>
                    <p><b>Pnone Number:</b> {contact.phone}</p>
                </div>
                <div className="flex items-center self-center gap-3">
                    <Link href={`/contacts/edit/${contact._id}`}
                     className="flex items-center gap-1 bg-blue-500 hover:bg-blue-700 text-white px-2 py-0.5 rounded-md cursor-pointer text-sm"><FiEdit size={12}/>Edit</Link>
