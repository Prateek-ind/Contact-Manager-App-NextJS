...
const ContactPage = async() => {
  ...
  const contacts = await getContacts(user?.id as string)
  console.log("FETCHED CONTACTS:", contacts)
  if(!contacts || contacts.length === 0){
    return <div className="text-gray-700">No contacts found. Please{" "} 
    <Link href="/contact/new" className="text-blue-500 underline">add</Link> 
    {" "}some contacts.</div>
  }
  ...
  return (
    ...
    <div className="container mx-auto p-4 flex items-center justify-between">
      ...
      <Link href={'/contact/new'} className="bg-blue-500 hover:bg-blue-700 text-white px-2 py-0.5 rounded-md cursor-pointer">Add Contacts</Link>
